...
static GLboolean yuv420sp_use(WX_GLES2_Renderer *renderer)
{
...
static GLsizei yuv420sp_getBufferWidth(WX_GLES2_Renderer *renderer, WX_SDL_VoutOverlay *overlay)
{
    if (!overlay)
        return 0;
...
static GLboolean yuv420sp_uploadTexture(WX_GLES2_Renderer *renderer, WX_SDL_VoutOverlay *overlay)
{
    if (!renderer || !overlay)
        return GL_FALSE;
...
WX_GLES2_Renderer *WX_GLES2_Renderer_create_yuv420sp()
{
    WX_GLES2_Renderer *renderer = WX_GLES2_Renderer_create_base(WX_GLES2_getFragmentShader_yuv420sp());
    if (!renderer)
        goto fail;
